...
\autocite{VTProgMisconceptions,VTDebug,VTTerminal}, the literature's
documented misconceptions play the role of those phenomenographic
observations; the instruments of \cref{sec:method-instruments} add the
...
The literature identifies the notional machine as a major challenge and
argues it should be an explicit learning objective
\autocite{Sorva2013NotionalMachines,DuBoulay1981BlackBox}; novice
...
Git needs a notional machine of its own: the commit graph with its
pointers---branches as movable references into the history, the staging
area between working tree and repository.
The \texttt{cd}-into-a-branch misconception \autocite{IsomottonenCochez2014}
is precisely what results when that machine is missing and the learner
substitutes the file-system model they already have.
